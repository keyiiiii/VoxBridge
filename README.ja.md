...
- **リアルタイムプレビュー** — 録音中にオーバーレイで文字起こし結果をリアルタイム表示
- **翻訳モード** — 日本語で話して英語で入力、またはその逆（JA→EN / EN→JA）
...
- **Ollama** (オプション) — テキスト整形・翻訳モードを使う場合のみ

### Ollama + Qwen 3 (オプション)

[Ollama](https://ollama.com/) はローカル LLM サーバーで、[Qwen 3](https://github.com/QwenLM/Qwen3) はその上で動く AI 言語モデル。VoxBridge は Qwen を使って音声認識結果を整形する（「えーと」「あのー」などのフィラー除去、句読点の追加）。**なくても動作する**（整形をスキップして STT の結果がそのまま入力される）。

**メニューバーからセットアップ（最も簡単）:**

Ollama が検出されない場合、メニューバーの **VB** に **Install Ollama...** が表示される。クリックするとダウンロードページが開く。インストール後、モデルが未ダウンロードの場合は **Download Qwen 3 model...** が表示され、クリックするとバックグラウンドでダウンロードが開始される（進捗はオーバーレイに表示）。
...
2. 録音中、オーバーレイに**リアルタイム文字起こしプレビュー**が表示される
3. キーを **離す** → 録音停止 → 文字起こし → 整形 → アクティブなアプリにテキスト入力
4. Terminal / iTerm2 等のターミナルアプリでは、自動で Enter も送信
5. 録音は **60 秒制限** (設定可能) — 残り 10 秒からカウントダウンが表示され、制限到達時に自動で処理される
...
Download Qwen 3 model...     (モデル未DL時のみ表示)
─────────
Launch at Login
─────────
Quit
...
- **Formatting** — テキスト整形の On/Off を切り替え、または翻訳モード（JA→EN / EN→JA）に切り替え。翻訳モードではローカル LLM を使って音声を別の言語に翻訳して入力する。Ollama やモデルが利用できない場合はグレーアウト。
...
`v*` タグを push、またはワークフローを手動実行すると、GitHub Actions が自動でビルド・リリースを作成する。Homebrew tap も自動更新される。
...
│   ├── translate_ja_en.txt     # 翻訳プロンプト (JA → EN)
│   └── translate_en_ja.txt     # 翻訳プロンプト (EN → JA)
├── resources/
...
├── .github/
│   └── workflows/
│       ├── build-release.yml   # リリース自動化 (タグ push → ビルド → GitHub Releases → Homebrew)
│       └── test.yml            # CI テスト (push / PR)
├── voxbridge/
│   ├── __init__.py
